...
`paperclip-page` publishes static page directories to a Zworker-controlled S3
bucket served through CloudFront. It is the durable Zworker-owned replacement
...
- Public content only.
- Dedicated uploader IAM identity, separate from Zworker attachment storage.
...
Recommended Zworker secret names:
...
`AWS_SECRET_ACCESS_KEY`. Do not reuse Zworker's internal S3 attachment/object
storage credentials.
...
export CLOUDFRONT_COMMENT="Zworker pages"
...
pnpm paperclipai skills create \
  ...
  --name "Zworker Page" \
  --slug paperclip-page \
  --description "Publish static pages to the Zworker pages host" \
...
2. Store new secret versions in Zworker Secrets.
