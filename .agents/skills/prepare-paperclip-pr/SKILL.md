...
description: Prepare a Zworker branch for PR with commits, template body, and checks.
---
# Prepare Zworker PR

The standard Zworker procedure for turning branch work into a reviewed,
...
* Do all PR work in a **git worktree** on a dedicated branch. The main
  `~/paperclip` checkout typically runs the live Zworker server — never
...
* Commit messages must end with exactly:
  `Co-Authored-By: Zworker <noreply@paperclip.ing>`
