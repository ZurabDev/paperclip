...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Zworker API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Zworker server is not running, the configured URL is wrong, or the request is being blocked before it reaches Zworker.",
    "",
    "Try:",
    "- Start Zworker with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Zworker is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
