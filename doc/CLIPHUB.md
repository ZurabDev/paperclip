...
ClipHub is the public registry where people share, discover, and download Zworker company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Zworker what a package registry is to a programming language. Zworker already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Zworker — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Zworker instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Zworker spec. It contains:
...
```
Downloads the template and creates a new company in your local Zworker instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Zworker, export your company as a template, then publish:
...
ClipHub is a **separate service** from Zworker itself. Zworker is self-hosted; ClipHub is a hosted registry that Zworker instances talk to.
...
| **Zworker CLI** | `paperclipai install`, `paperclipai publish`, `paperclipai cliphub sync` — built into Zworker |
| **Zworker UI** | "Browse ClipHub" panel in the Zworker web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Zworker) |
| Backend | TypeScript + Hono (consistent with Zworker) |
| Database | PostgreSQL |
...
5. Zworker creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Zworker until it works well
...
## Relationship to Zworker

ClipHub is **not required** to use Zworker. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Zworker what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in Zworker web UI
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Zworker's own philosophy)
