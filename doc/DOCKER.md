...
Run Zworker in Docker without installing Node or pnpm locally.
...
Zworker server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Zworker data persists in `paperclip-data`.
...
For authenticated deployments, set one canonical public URL and let Zworker derive auth/callback defaults:
...
Zworker URL, sign in or create an account, then choose `Claim this instance`
...
- Adapter environment checks in Zworker will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run Zworker + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/paperclip.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/paperclip.container` | Zworker server — joins the pod, connects to Postgres at `127.0.0.1` |
...
- Containers in a pod share `localhost`, so Zworker reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `paperclip-pgdata` named volume.
- Zworker data persists at `~/.local/share/paperclip`.
...
- Smoke script defaults host port to `3131` to avoid conflicts with local Zworker on `3100`.
...
- Zworker data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/paperclip` (quadlet).
