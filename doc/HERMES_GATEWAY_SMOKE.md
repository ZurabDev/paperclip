...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or Zworker. It
...
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed Zworker agent key and is written
  `chmod 600`.
...
- `PAPERCLIP_API_URL`: Zworker URL used by the operator shell.
- `PAPERCLIP_API_URL_FOR_HERMES`: Zworker URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the Zworker
  adapter, reachable by the Zworker server.
...
## Docker Desktop or Linux host Zworker

Use this when Zworker runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as Zworker

Use this when Zworker is a container on a Docker network and the Hermes smoke
...
name. The unsafe HTTP flag is required because Zworker stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network Zworker

Use this when Zworker is exposed on a private IP or tailnet address and the
...
Use this when Zworker should talk to Hermes through a TLS hostname. The smoke
...
`HERMES_GATEWAY_API_BASE_URL` to the URL Zworker should store on the adapter.
