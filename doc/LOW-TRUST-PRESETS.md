...
Zworker ships core trust preset names so containment decisions are enforced in
...
scope for risky access, Zworker fails closed.
...
agent can read or mutate through the Zworker API and prevents raw untrusted
...
Managed `low_trust_review` runs fail closed unless Zworker can enforce the
runtime boundary:
...
local review, but Zworker-managed low-trust execution requires a sandboxed
