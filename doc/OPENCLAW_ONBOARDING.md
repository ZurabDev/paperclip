...
3. In Zworker UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Zworker UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Zworker invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Zworker with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Zworker UI that new issue exists.
