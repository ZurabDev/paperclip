...
| Subject | The app/workspace or Zworker user on whose behalf a credential is requested. | The calling agent. |
...
| Trigger | Provider-origin event definition that starts governed Zworker work. | An unauthenticated webhook handler. |
| Connector service | Zworker-operated relay for managed OAuth callbacks, credential custody, and webhook intake at `connect.paperclip.ing`. | Zworker ID or the per-company broker. |
...
| Skill | Agent instruction package that calls governed connections through Zworker. |
...
- Inbound clients use scoped Zworker auth; outbound vendor calls use the Apps
