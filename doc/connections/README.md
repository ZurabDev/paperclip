...
Post-read action: classify a new integration request, pick the right Zworker
...
- **D5: inbound stays thin.** External clients that call Zworker use scoped
  Zworker tokens and existing profiles/rules. They do not get a separate
  permission model.
...
Zworker run/session token; the server or broker resolves the connection,
...
| **P2. Connections (Apps)** | *What may your agents touch?* | Zworker App instances (`tool_connections`), acquired via the **connect broker** for hosted + self-hosted | Rich-scope, long-lived resource tokens in the **instance's** encrypted vault; per-agent grants; ask-first on writes |
| **P3. Login with Zworker** | *Who may authenticate against us?* | `paperclip-id` OIDC provider + DB-backed client registry | Our ES256 ID/access tokens issued *by* us to registered RPs (instances, the broker, future third parties) |
...
| --- | --- | --- |
| Zworker App instances | P2 | **"Connections"** |
