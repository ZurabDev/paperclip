...
2. **Connection operations are company-scoped and brokered by Zworker.**
...
- Zworker objects mutated by integrations: issues, comments, documents,
...
- Route webhook payloads only to the owning company/connection. Ignore request
  ...
  connection.
- Apply resource filters before storing external mappings or mutating Zworker.
- For revoked/disabled connections, acknowledge where provider semantics require
  it, log the filtered/drop outcome, and do not mutate Zworker state.
...
- Provider OAuth/app-installation scopes may be broader than Zworker resource
  filters. Zworker must enforce the narrower internal filter.
