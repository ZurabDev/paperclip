...
This guide describes the current, implemented way to create a Zworker plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Zworker app.
...
- The host provides a small shared React component kit through
  `@paperclipai/plugin-sdk/ui`; use it for common Zworker controls before
...
Zworker owns inline markdown rendering; plugins must not return React, HTML,
...
Outside this monorepo, the scaffold snapshots `@paperclipai/plugin-sdk` from the local Zworker checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/paperclip/packages/plugins/sdk` if you have more than one Zworker checkout.
...
## Managed Zworker resources

Plugins that provide durable Zworker business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Zworker agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Zworker issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Zworker managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Zworker heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Zworker issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Zworker records that the
...
Zworker-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Zworker UX patterns to reduce drift and
