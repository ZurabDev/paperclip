# Zworker Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Zworker's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Zworker app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Zworker HTTP APIs directly.
...
Zworker plugin design is based on the following assumptions:

1. Zworker is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Zworker business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Zworker core.
...
2. Let plugins add major capabilities without editing Zworker core.
...
The single Zworker deployment an operator installs and controls.

### 5.2 Company

A first-class Zworker business object inside the instance.
...
A trusted in-process extension loaded directly by Zworker core.
...
An installable instance-wide extension package loaded through the Zworker plugin runtime.
...
Zworker has two extension classes.
...
Zworker already has a concrete workspace model:
...
Plugins live under the Zworker instance directory.
...
Zworker should add CLI commands:
...
Plugins may contribute tools that Zworker agents can use during runs.
...
- Zworker server starts one worker process per installed plugin
...
Receives one typed Zworker domain event.
...
Trusted orchestration plugins can create and update Zworker issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Zworker resources rather than using private plugin state:
...
6. For recurring business workflows that should create visible Zworker work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
Zworker-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Zworker navigation because those can force a full document reload.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that stores the shared `{ type: "secret_ref", secretId, version? }` object shape and resolves through the Zworker secret provider system rather than a plain text input
...
1. Core Zworker data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Zworker objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Zworker product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Zworker later decides git state is core product data, it should become a first-party table too
...
3. Secret refs resolve through the existing Zworker secret provider system using explicit `companyId`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Zworker server. This is a normative requirement, not optional.
...
For developing a plugin against a running Zworker instance:
...
Zworker should not implement a generic in-process hook bag modeled directly after local coding tools.

Zworker should implement:
...
That is the complete target design for the Zworker plugin system.
