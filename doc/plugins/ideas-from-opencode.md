...
Zworker V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Zworker-shaped design.

Assumption for this document: Zworker is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Zworker objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Zworker.

The main conclusion is:

- Zworker should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Zworker should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Zworker should use multiple extension classes instead of one generic plugin bag:
...
If Zworker does this well, the examples you listed become straightforward:
...
Relevant Zworker files reviewed for current extension seams:
...
It is too dangerous for Zworker core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Zworker — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Zworker should absolutely do this.
...
Zworker should copy this discipline.
...
Zworker should adopt this style for plugin actions, automations, and UI schemas.
...
Zworker can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Zworker too.

## What Zworker Should Not Copy Directly
...
Zworker is a control plane for an operator-managed instance with company objects.
...
Zworker is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Zworker should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For Zworker, equivalent hooks into:
...
Zworker should not allow plugins to silently replace:
...
For Zworker it would be risky because it combines:
...
Zworker should require an explicit operator install step.

## Why Zworker Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Zworker |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Zworker instance |
...
That means Zworker should borrow the good ideas from `opencode` but use a stricter architecture.

## Zworker Already Has Useful Pre-Plugin Seams

Zworker has several extension-like seams already:
...
This is good news.
Zworker does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Zworker Plugin Model
...
Zworker already has this pattern implicitly:
...
- `plugin` means integration code that talks to Zworker through a typed plugin protocol and capability model
...
- invoke explicit Zworker actions through the API
...
Zworker should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Zworker objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Zworker already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Zworker too.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Zworker instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Zworker achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Zworker server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Zworker's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Zworker
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Zworker entities
    });
...
Any Zworker plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Zworker objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of Zworker itself
...
If a concept is becoming part of Zworker's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Zworker itself
...
### 3. Scope plugin data to Zworker objects before adding custom schemas

A lot of plugin data naturally hangs off existing Zworker objects:
...
If Zworker eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Zworker.
- Board reviews the diff after a run without leaving Zworker.
...
This plugin syncs Zworker work with Linear. It is useful for:

- importing backlog from Linear
- linking Zworker issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Zworker and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Zworker.
- Zworker issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs Zworker issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Zworker issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Zworker task governance
...
- Plugin settings:
  ...
  - whether new Zworker issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Zworker issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Zworker.
- GitHub webhooks update status/comment state in Zworker.
- A PR is linked back to the Zworker issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside Zworker. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Zworker issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Zworker plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Zworker dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Zworker issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Zworker core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into Zworker. It is useful for:
...
- Failed payment spikes or churn events can generate Zworker issues for follow-up.
...
- Stripe data should stay additive to Zworker core
...
Zworker should not implement "an OpenCode-style generic in-process hook system."
Zworker should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Zworker
