...
This spec defines how Zworker actually runs agents while staying runtime-agnostic.
...
1. Zworker is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Zworker manages lifecycle, and on exit Zworker parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Zworker skill text block)
