...
For `claude_local` and `codex_local`, Zworker assumes the CLI is already installed and authenticated on the host machine.
...
Zworker stores resumable session state per `(agent, taskKey, adapterType)`.
