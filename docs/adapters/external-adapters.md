...
summary: Build, package, and distribute adapters as plugins without modifying Zworker source
---

Zworker supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Zworker's source code.
...
| Distribution | Ships with Zworker | Published to npm or linked via `file:` |
| Updates | Requires Zworker release | Independent versioning |
...
export async function execute(
  ...
  // 2. Build environment with Zworker vars injected
...
Local adapters are symlinked into Zworker's adapter directory. Changes to the source are picked up on server restart.
...
Other Zworker users can then install your adapter by package name from the UI or API.
