...
- Running a Python script that calls the Zworker API
...
1. Zworker spawns the configured command as a child process
2. Standard Zworker environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Zworker API and perform work.
