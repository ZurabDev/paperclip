...
Zworker `external_reference` secrets without copying plaintext into
Zworker.
...
`ListSecrets` only and import stores a Zworker external reference plus
...
remote metadata, or copies secret plaintext into Zworker.
...
- `conflict`: the suggested Zworker `name` or `key` is already in use.
...
Zworker-managed AWS secret through a broad runtime role.
...
Each row may include an optional Zworker `description` entered during review;
...
the Zworker runtime role can list the AWS secret but lacks
