...
To link a provider-owned secret without copying the value into Zworker, create
...
Zworker stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Zworker `company_secrets`.
...
Remote import links existing AWS Secrets Manager entries into Zworker as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Zworker.
...
- `duplicate`: a Zworker secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Zworker's own managed namespace are
blocked as external references; use the Zworker-managed secret flow for those
resources instead.
...
Zworker `name`, `key`, optional Zworker `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Zworker
...
- `imported`: Zworker created an active `external_reference` secret and one
...
real value into the agent process environment. Zworker's custody guarantees
...
empty string or another user's value. Zworker records value-free access
