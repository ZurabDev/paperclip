...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Zworker.
...
- Zworker must not require extra top-level fields for skill validity
- Zworker-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For Zworker, the preferred fidelity extension is:
...
- issue/task Zworker-only metadata
...
- Zworker tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Zworker shape:
...
Additional rules for Zworker exporters:
...
- omit empty and default-valued Zworker fields when possible
...
- Zworker exporters should emit `.paperclip.yaml` as a sidecar by default
...
Zworker can map this spec to its runtime model like this:
...
- Zworker extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Zworker-specific fidelity

Inline Zworker-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than Zworker.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Zworker.

## 20. Cutover

Zworker should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Zworker, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
