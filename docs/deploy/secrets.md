...
Zworker encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Zworker protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Zworker does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Zworker can no longer guarantee
...
At runtime, Zworker resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Zworker injects its
...
Zworker stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from Zworker secrets.
...
- Keep Zworker-managed AWS values under the Zworker managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  Zworker-managed flow when Zworker should create and rotate the value.

### IAM Caveats

Zworker enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the Zworker runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the Zworker managed prefix,
...
refs. Zworker can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside Zworker.
...
Zworker best-effort enforces `0600` permissions when it creates or loads the
...
Provider-owned secrets can be linked without copying values into Zworker by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Zworker stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Zworker Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Zworker company secrets**. Allowed credential sources are
workload identity attached to the Zworker server (instance profile, IRSA, ECS
...
Zworker `external_reference` secrets. This is a metadata-only link: Zworker
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Zworker name and key, then
   submit.
...
cursors. Do not expect Zworker to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Zworker runtime
...
- Keep managed create/rotate/delete permissions scoped to the Zworker
...
dedicated Zworker runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Zworker
...
- `aws_secrets_manager`: back up Zworker's database for vault metadata
  ...
  same Zworker company at the same AWS namespace and confirming the runtime
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Zworker. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Zworker
...
For Zworker Cloud, provision the server runtime IAM role/workload identity,
...
Zworker secrets. Bootstrap material belongs in infrastructure IAM/workload
...
Use the CLI command for normal operations because it goes through the Zworker
...
the value custodian; Zworker stores metadata and provider version references,
