...
summary: Run Zworker with Tailscale-friendly bind presets and connect from other devices
---

Use this when you want to access Zworker over Tailscale (or a private LAN/VPN) instead of only `localhost`.

## 1. Start Zworker in private authenticated mode
...
From the machine running Zworker:
...
## 3. Open Zworker from another device

Use the Tailscale IP or MagicDNS host with the Zworker port:
...
If you access Zworker with a custom private hostname, add it to the allowlist:
