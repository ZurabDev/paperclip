...
The **Skills Store** is Zworker's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with Zworker | The `@paperclipai/skills-catalog` package |
...
- **`bundled`** — first-party Zworker skills (e.g. `issue-triage`, `task-planning`,
...
|---|---|---|
| `catalog` | Zworker / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: Zworker ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when Zworker can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
Zworker approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and Zworker fetches and imports it. Accepted forms include:
