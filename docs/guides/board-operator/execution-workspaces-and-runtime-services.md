...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Zworker.

Zworker now presents this as a workspace-command model:
...
- Zworker does not automatically restart workspace services on server boot — services only come back up when the next run (or a manual start) brings them up.
...
2. Zworker realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Zworker persists execution-workspace metadata such as paths, refs, and provisioning settings.
