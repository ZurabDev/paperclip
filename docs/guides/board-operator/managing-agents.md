...
Use `hermes_local` when Zworker should start the local Hermes CLI. Use
`hermes_gateway` when Hermes is already running as an API server and Zworker
...
Zworker validates the selected model against live `opencode models` output.
