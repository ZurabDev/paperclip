...
Status cards use SQL change detection before spending model tokens. Zworker reruns the stored query set on scheduler ticks, compares the result with the previous fingerprint, and marks meaningful additions, removals, or configured field changes as pending.

- **Manual** is the default. Changes make the card stale, but Zworker never starts an automatic update.
