...
How to get OpenClaw running in a Docker container for local development and testing the Zworker OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Zworker includes an end-to-end join smoke harness:
...
- probes and prints a Zworker host URL that is reachable from inside OpenClaw Docker
...
If your Zworker deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Zworker server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Zworker URL (typically `http://host.docker.internal:3100`).
- If Zworker rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart Zworker and rerun the smoke script.
