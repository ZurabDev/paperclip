...
Add first-class **documents** to Zworker as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Zworker workflow/docs.
...
Update the Zworker agent workflow so planning no longer edits the issue description.
