# ClipHub: Marketplace for Zworker Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Zworker configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Zworker-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Zworker company (agents, projects, governance) |
...
A complete Zworker company configuration:
...
Single-agent configurations designed to plug into a Zworker org:
...
Portable skill files that any Zworker agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Zworker's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Zworker version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Zworker company
...
| `POST` | `/api/listings/:id/install` | Install to Zworker company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Zworker company (or create new)
  → ClipHub API calls Zworker API to:
  ...
  → Redirect to Zworker dashboard with new team running
```
...
### 5.3 Creator: Export from Zworker → Publish

```
Running Zworker company → "Export as Blueprint" (CLI or UI)
  → Zworker exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Zworker brand blue/purple)
...
| Install flow | One-click deploy to Zworker company |
...
{
  "targetCompanyId": "uuid",         // Existing Zworker company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Zworker
- **Backend**: Node.js API (or extend Zworker server)
- **Database**: Postgres (can share Zworker's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Zworker auth (or OAuth2)

### 9.2 Integration with Zworker

ClipHub can be:
- **Option A**: A separate app that calls Zworker's API to install blueprints
- **Option B**: A built-in section of the Zworker UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Zworker UI and API.
...
- [ ] Install flow: blueprint → Zworker company
...
- [ ] "Export from Zworker" CLI command
