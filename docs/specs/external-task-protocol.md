# Zworker External Task Protocol Specification

Status: Draft v1 (provider-agnostic)

Purpose: Define the protocol by which Zworker interoperates with external task managers (Linear,
Jira, Asana, Notion, Trello, GitHub Issues, and others) so that external tasks can drive Zworker
agent work and Zworker work is visible in the external tool, without requiring an external user
account for every Zworker agent.
...
- `Provider` — an external task manager product (Linear, Jira, Asana, Notion, Trello, GitHub
  Issues, ...).
- `Connector` — an implementation of this protocol for one provider, hosted in the Zworker
  plugin runtime.
- `External task` — a task record that lives in the provider.
- `Zworker issue` — the native Zworker task record.
- `Host` — the Zworker control plane: companies, agents, issues, checkout, heartbeat runs,
  execution workspaces, budgets, approvals, blockers, issue documents, and work products.

## 1. Problem Statement

Teams already run their work in external task managers. Zworker runs companies of agents. Today
these worlds only meet through manual copy/paste: a human reads a Linear issue, rewrites it as a
Zworker issue, and later rewrites the outcome back into Linear.

This protocol defines the contract that lets an external task manager act as a **task source and
collaboration surface** for a Zworker company, while Zworker remains the **execution and
...
- It defines how an external task becomes (or links to) exactly one Zworker issue, with
  idempotent sync state that survives retries, webhook replays, and restarts.
- It defines how external assignment routes to Zworker agents without creating a provider user
  per agent.
...
- Let an external task manager act as a task source: active external tasks create or wake
  Zworker issues under configured policy.
...
- Route external assignment intent to Zworker agents through deterministic, configurable rules.
- Reuse Zworker's existing execution semantics — checkout as claim, heartbeat runs, execution
...
- Replacing the Zworker board with a clone of any provider's UI.
- Real-time field-level collaborative editing between Zworker and a provider.
...
- Creating a provider user account per Zworker agent (supported as an OPTIONAL mapping, never
  required).
- Letting external systems bypass Zworker governance: checkout, budgets, approvals, blockers,
...
2. `Task Link Store`
   - Persists the 1:1 association between an external task and a Zworker issue.
...
3. `Inbound Sync Engine`
   ...
   - Creates/updates Zworker issues and imports comments with provenance.
   - Runs scheduled reconciliation to repair drift.

4. `Routing Engine`
   - Resolves external assignment intent into a Zworker assignee using the ordered rule
...
2. `Layer 2 — Task Link` (REQUIRED)
   - Durable external-task ↔ Zworker-issue association with sync state.

3. `Layer 3 — Execution Orchestration, Symphony Profile` (RECOMMENDED)
   - Active external states drive Zworker issue creation/wake; Zworker checkout/run/retry
...
| Claim | Zworker issue checkout + execution lock (Section 9.2) |
...
A connector MUST perform all Zworker issue/comment/status mutations through the capability-gated
...
Field names use camelCase, matching Zworker API conventions. (Symphony uses snake_case; the
...
The durable 1:1 association between one external task and one Zworker issue.
...
- `companyId`, `projectId`, `goalId`, `issueId` (strings; `issueId` is the linked Zworker issue)
- `handoffHistory` (list of `{issueId, endedAt, cause}`; empty for a link that has never moved)
  - Prior Zworker issue associations retained when reopened work is handed to a follow-up.
...
- `lastPaperclipFingerprint` (string or null)
  - Hash of the last Zworker-side state this connector projected outbound.
...
- `displayName` (string)
- `mappedUserId` (string or null) — Zworker user, if mapped.
- `mappedAgentId` (string or null) — Zworker agent, if explicitly mapped (OPTIONAL feature).
...
- `import_only` — external → Zworker only.
- `export_only` — Zworker → external only.
...
- `paperclip_controlled` — Zworker governance decides when agents run; external state changes
  inform but do not command execution.
- `symphony_compatible` — active external states create/wake Zworker work automatically per
...
- `Idempotency key` for outbound effects — `(linkId, effectKind, paperclipSourceId)` (e.g. the
  Zworker comment ID being mirrored). Retried jobs MUST NOT double-post.
...
- Person references in body text SHOULD be converted to plain display names; raw provider mention
  syntax MUST NOT leak into Zworker issue bodies where it could be misparsed as Zworker
  mentions.
...
1. `Import` (`originSide=external`)
   - An external task is selected (manually, or automatically under `runMode=symphony_compatible`
     state mapping) and a Zworker issue is created from its projection.
...
2. `Export` (`originSide=paperclip`)
   - A Zworker issue is projected into the provider via `createTask`.
   - REQUIRED only for connectors declaring write capability.

3. `Manual link` (`originSide=manual_link`)
   - Operator pairs an existing external task with an existing Zworker issue.
...
2. Zworker backlink already present on the external task (custom field/comment marker).
3. External URL/key reference already present on a Zworker issue in the same company.
...
- When an external task is deleted/archived: the connector MUST NOT delete or cancel the Zworker
  issue. Default behavior: mark the link `conflict` (`statusDetail: external task deleted`),
  comment on the Zworker issue, and let the assignee/operator decide. `import_only` links with
  no Zworker-side activity MAY auto-unlink.
- When a Zworker issue is cancelled/deleted: outbound policy decides whether to comment and/or
...
## 7. Inbound Synchronization (External → Zworker)
...
- Imported comment content is **untrusted input**: it MUST be clearly attributed in the issue
  thread, MUST NOT be interpreted as Zworker system/agent instructions, and mention-like syntax
  MUST NOT trigger Zworker mention semantics except via the strict command path (Section 8.2).
...
### 7.4 External State Transitions Against Zworker Work
...
- `active -> backlog` or `active -> blocked`: the connector SHOULD surface this on the issue; under
  ...
  Zworker-native obligations (approvals in flight, blockers) are unaffected.
- `* -> terminal` while the Zworker issue is active (checked out, running, in review, or carrying
...
- `terminal -> active` (reopen): if the linked issue is terminal, policy chooses between creating a
  follow-up Zworker issue linked to the same external task (RECOMMENDED default) or reopening,
...
External transitions MUST NOT directly set Zworker issue status; they translate into host-level
...
External assignment intent resolves to a Zworker assignee through an ordered pipeline. The first
...
2. `custom_field` — provider custom field naming an agent (example: `Zworker Agent = CodexCoder`).
3. `label` — namespaced label (example: `pc:agent/codexcoder`, `pc:route/qa`).
4. `state_lane` — provider state/lane mapped to a route (example: Trello list `Zworker: QA`).
...
- The connector MUST reply (externally, if writes allowed; on the Zworker issue otherwise) with
...
- Zworker agents MUST NOT be required to exist as provider users.
- Outbound writes use one connector service account per connector instance (Section 10.4), with
  agent attribution carried in message content ("CodexCoder via Zworker").
- `user_mapping` rules MAY map specific provider users to Zworker users (for `assign it back to
...
Symphony's poll-tick loop becomes, in Zworker:
...
- Zworker issue checkout is the claim. One agent owns an issue at a time; checkout conflicts
...
1. Zworker project workflow document (RECOMMENDED MVP form).
...
## 10. Outbound Synchronization and Write Policy (Zworker → External)
...
| --- | --- | --- |
| `postBacklink` | on | Zworker URL on the external task (field or pinned comment). |
...
| `mirrorAgentComments` | off | Full Zworker comment thread mirrored externally. |
...
Zworker issue status projects to provider states through the same mapping table as Section 7.2,
inverted, with these rules:

- Projection only moves the external task between states the operator mapped; unmapped Zworker
...
- `done` projects to the mapped terminal state only when `fieldPolicy.state` permits
  Zworker-side closure; otherwise it posts a completion comment and leaves the transition to
  external users.
...
- Outbound writes use the connector instance's service account. Message bodies MUST carry agent
  attribution and the Zworker issue link.
...
- `keep_paperclip` (push Zworker value outbound)
...
On the provider side, a linked task SHOULD show: the Zworker backlink, current Zworker
...
Destructive cascade is prohibited: external deletion/archival never deletes Zworker work
(Section 6.4).
...
- Containers: board, list. Envelope: list id, checklists, members, power-up metadata, card
  position.
- State mapping: lists are lanes; map lists→categories (`Zworker: QA` style lanes also drive
  `state_lane` routing).
