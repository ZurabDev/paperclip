...
describe("local process sandbox", () => {
  ...
  it.runIf(process.platform === "linux")("forwards allowed proxy targets with a deep TMPDIR and rejects other hosts", async () => {
    ...
    try {
      ...
      await expect(request("http://example.com/")).resolves.toEqual({
        ...
        body: '{"error":{"code":"network_target_denied","message":"Network target denied by Zworker sandbox policy."}}\n',
      });
      ...
      expect(connectResponse).toContain(
        '{"error":{"code":"network_target_denied","message":"Network target denied by Zworker sandbox policy."}}\n',
      );
    } finally {
  ...
  it.runIf(process.platform === "linux")("always permits trusted Zworker control-plane URLs", async () => {
