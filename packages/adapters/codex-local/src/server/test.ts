        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            hint: probeApiKey
              ...
              : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so Zworker writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
          });
        } else {
          checks.push({
