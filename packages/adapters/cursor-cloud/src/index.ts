...
Use when:
- You want Zworker to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across Zworker heartbeats
- You want Zworker to keep task state while Cursor handles remote code execution
...
Notes:
- Zworker reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each Zworker heartbeat maps to a Cursor run on that durable agent.
- Zworker injects PAPERCLIP_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- Zworker remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
