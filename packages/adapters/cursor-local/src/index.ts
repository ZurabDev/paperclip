...
Use when:
- You want Zworker to run Cursor Agent CLI locally as the agent runtime
...
- Zworker auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- Zworker auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
