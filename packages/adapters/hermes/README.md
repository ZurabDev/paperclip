# Zworker Adapters for Hermes Agent

A [Zworker](https://paperclip.ing) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a Zworker company.
...
Choose `hermes_local` when Zworker and Hermes run on the same trusted host
and Zworker should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both Zworker-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so Zworker renders proper tool cards with status icons and expand/collapse
...
This package ships with Zworker core as the built-in `hermes_local` and
...
Zworker. The external package can override either built-in Hermes adapter
while it is enabled:
...
Zworker core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in Zworker

In the Zworker UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. Zworker injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in Zworker
...
`http://127.0.0.1:9119/chat`, Zworker maps it to
`http://127.0.0.1:9119/api` before calling Hermes API routes. `/chat` and the
dashboard root are browser UI routes; Zworker tests `/api/health` and starts
...
for existing plugin installs. New installs and built-in Zworker registrations
should use `@paperclipai/hermes-paperclip-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives Zworker runtime identity through environment variables:
...
`Authorization: Bearer $PAPERCLIP_API_KEY` on Zworker API requests and
...
starts in Hermes and asks Hermes to create or update Zworker work. This is not
the same as Zworker waking Hermes through `hermes_local` or `hermes_gateway`.

Configure Zworker access in Hermes env/profile secrets, not prompt text:
...
Create issues in Zworker and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to Zworker
...
| `paperclipApiUrl` | string | `http://127.0.0.1:3100/api` | Zworker API base URL |
...
| `{{paperclipApiUrl}}` | Zworker API base URL |
...
6. **Reports** results back to Zworker with cost, usage, and session state
...
- **Zworker-managed skills** — bundled with the adapter, togglable from the UI
...
Zworker UI can display both managed and native skills in one view.
...
- [Zworker](https://github.com/paperclipai/paperclip) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [Zworker Docs](https://docs.paperclip.ing) — Zworker documentation
