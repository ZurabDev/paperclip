...
description: Create, comment on, update, and list Zworker tasks from Hermes using scoped Zworker API credentials.
---

# Zworker Task Bridge

Use this skill when a Hermes-originated request needs to create or update Zworker work directly. This is the Hermes-to-Zworker direction, separate from Zworker waking Hermes through the `hermes_local` or `hermes_gateway` adapter.
...
- `PAPERCLIP_BRIDGE_API_KEY` - a Zworker agent API key created with `scope.kind = "task_bridge"`.
...
- `PAPERCLIP_RUN_ID` - sent as `X-Paperclip-Run-Id` on mutating requests when Hermes is running inside a Zworker heartbeat.

Never print or paste API keys. The helper reads credentials from environment variables and only prints response summaries. Do not put a normal claimed agent API key in an internet-facing Hermes runtime; normal keys can use broad same-company Zworker routes.

## Create a Bridge Key

Create the key from a board-authenticated Zworker API session and store the returned token once:
...
`create-task` defaults to assigning the task to the authenticated Hermes agent so the work is immediately actionable. Use `--unassigned` to create backlog work instead. Use `--assignee-agent-id <uuid>` only when the Zworker API key has permission to assign work to that agent.
...
- Let Zworker activity logging come from the normal API endpoints; do not write local logs that include credentials.
...
- Use `list-assigned` before creating duplicate work when the user asks about current Zworker assignments.
