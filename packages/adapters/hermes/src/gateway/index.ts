...
- Zworker should create Hermes runs through POST /v1/runs and observe them through SSE events.
- You need remote Hermes session continuity with X-Hermes-Session-Key.

Don't use when:
- Hermes should run as a local child process on the Zworker host; use hermes_local instead.
- The Hermes API server is not enabled or is only reachable over an unsafe public HTTP endpoint.
- You need Hermes-originated Zworker task creation only; that is a Zworker skill/API bridge, not this wake adapter.
...
- paperclipApiUrl (string): Zworker API URL reachable from the Hermes host. This is not a credential.
...
- Sends Idempotency-Key equal to the Zworker run id for correlation only; Hermes v0.16.0 did not dedupe duplicate creates.
...
- Do not put Hermes apiKey or Zworker bearer tokens in prompts, comments, logs, or result JSON.
