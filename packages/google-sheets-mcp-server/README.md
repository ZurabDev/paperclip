...
First-party MCP server for Google Sheets API v4. It can run as a Zworker
`local_stdio` gallery connection or as a local Streamable HTTP server for
Zworker's `remote_http` connect-by-link flow.
...
## Zworker `local_stdio` Test Path

Use the Google Sheets gallery app when you want Zworker to supervise the
server as a stdio MCP process:

1. Configure Zworker's Google Sheets service account environment so the
...
2. Share every spreadsheet you want to test with the service account's
   `client_email`.
3. In Zworker, open the tool app gallery, choose **Google Sheets**, and paste
   one or more Google Sheets links.
4. Save the app connection. Zworker creates a `local_stdio` connection using
...
Then in Zworker, choose the remote HTTP or "connect with a link" path, paste
...
`GOOGLE_SHEETS_SPREADSHEET_IDS`), not a per-connection Zworker gallery wizard
