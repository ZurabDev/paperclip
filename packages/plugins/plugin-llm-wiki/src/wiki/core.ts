...
function assertPaperclipSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Zworker source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Zworker ingestion cap of ${max} characters.`);
  }
}
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Zworker source directly if a human needs the unredacted material.",
    ].join("\n"),
...
function evaluatePaperclipProfilePolicy(input: {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Zworker ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Zworker ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Zworker sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Zworker ingestion policy denied ${purpose}: Zworker ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Zworker ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
...
async function validatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Zworker ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_PAPERCLIP_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Zworker ingestion profile sources exceed the hard cap of ${MAX_PAPERCLIP_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
...
export async function updatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Zworker ingestion profile for ${space.displayName}`,
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  ...
  return [
    ...
    paperclipDerived
      ? "- Zworker-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Zworker-derived pages into a non-default space."
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_PAPERCLIP_DISTILLATION_FAN_OUT) {
    throw new Error(`Zworker ingestion fan-out exceeds the hard cap of ${MAX_PAPERCLIP_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
...
function sourceRefLabel(ref: PaperclipSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Zworker source");
...
function standupPageContents(input: {
  ...
  const title = input.project?.name ?? input.rootIssue?.title ?? "Zworker Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Zworker work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Zworker issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Zworker before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
export async function distillPaperclipProjectPage(ctx: PluginContext, input: PaperclipProjectPageDistillationInput) {
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordPaperclipDistillationOutcome(ctx, {
      ...
      warning: "Skipped low-signal Zworker source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Zworker source window."],
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordPaperclipDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Zworker source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Zworker source hash."],
      patches: [] as PaperclipDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Zworker Project";
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Zworker distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Zworker issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Zworker comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Zworker document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
