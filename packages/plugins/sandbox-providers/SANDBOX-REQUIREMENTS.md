...
must meet this contract. The Zworker runtime does not build the environment at
...
Zworker probes each CLI before launch. Zworker uses the same detection
...
Zworker probes each CLI with `command -v <cmd>`. Zworker fails loudly when
...
- The Zworker runtime never modifies the login profile. The runtime never
  writes a profile file. The runtime never writes an rc file.
- The Zworker runtime never sources `nvm` on the exec path.
