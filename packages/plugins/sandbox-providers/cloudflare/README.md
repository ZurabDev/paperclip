...
Published Cloudflare sandbox provider plugin for Zworker.

This package lives in the Zworker monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. Operators can install it from the Plugins page by package name, and the host will fetch its dependencies at install time without adding lockfile churn to the Zworker repo.
...
Pasted auth tokens are stored by Zworker as company secrets because the manifest marks `bridgeAuthToken` as a `secret-ref` field.
