...
It exposes a small authenticated JSON API under `/api/paperclip-sandbox/v1` and translates Zworker lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Zworker does not inherit wedged background processes
...
After deploy, configure Zworker with:
