...
Official TypeScript SDK for Zworker plugin authors.
...
- Plugin UI bundles run as same-origin JavaScript inside the main Zworker app. They can call ordinary Zworker HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Zworker instance at runtime.
...
- The host ships a small shared React component kit through `@paperclipai/plugin-sdk/ui`. Use it for native Zworker controls; custom React and CSS are still supported.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Zworker information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
metadata. Slow or failing plugins are isolated: Zworker logs the failure and
...
Routes Zworker-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.
...
Avoid raw same-origin `href`s or `window.location.assign()` for Zworker-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
used by Zworker issue comments and documents:
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/paperclip|Zworker]]."}
  enableWikiLinks
...
or configure work against Zworker entities. Both are controlled components and
