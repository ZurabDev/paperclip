...
Use this reference when the request is to generate Zworker capsule graphics, capsule identicons, or reusable visual assets.
...
Use this when the task needs reproducible generated capsule art beyond the small embedded website tool. The contract here is intentionally mirrored into Zworker so agents can proceed if the original prototype repository is renamed, deleted, or private.
...
If an implementation needs source code, prefer the Zworker website embedded generator or a Zworker-owned tool. Use any external prototype link only as optional historical context, not as required task input.

Palette caution:

- The `duotones` palette matches the website 12-preset capsule palette.
- Experimental palettes such as vaporwave, cyberpunk, ocean, and jewel are generator options, not canonical Zworker brand palettes.
- Do not use experimental palettes when the request asks for strict Zworker brand work.
...
- Divergence from canonical Zworker rendering:
```
