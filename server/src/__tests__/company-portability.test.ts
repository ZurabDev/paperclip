...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "Zworker coordination skill",
        markdown: "---\nname: paperclip\ndescription: Zworker coordination skill\n---\n\n# Zworker\n",
        sourceType: "github",
  ...
  it("exports referenced skills as stubs by default with sanitized Zworker extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Zworker"');
  ...
  it("exports agent permission grants through the Zworker extension and manifest", async () => {
    const db = {
  ...
  it("exports default sidebar order into the Zworker extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("# Zworker");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Zworker frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Zworker",
      },
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      ...
      target: { mode: "new_company", newCompanyName: "Imported Zworker" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("exports routines as recurring task packages with Zworker routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("pauses imported agents and routines when pauseAutomations is requested", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Zworker"', "---", ""].join("\n"),
  ...
  it("imports recurring tasks without a project or assignee as paused routines", async () => {
    ...
    const request = {
      ...
      target: { mode: "new_company" as const, newCompanyName: "Imported Zworker" },
  ...
  it("imports a vendor-neutral package without .paperclip.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Zworker");
  ...
  it("rejects packages produced by a newer Zworker", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    await expect(portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
    }, "user-1")).rejects.toThrow(/newer Zworker/);
    expect(issueSvc.importIssues).not.toHaveBeenCalled();
  });
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Zworker needs a disposition before this issue can continue.",
        presentation,
        metadata,
