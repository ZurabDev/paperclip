...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Zworker Agent Onboarding");
    ...
    expect(text).toContain("Suggested Zworker base URLs to try");
    ...
    expect(text).toContain("Hermes-originated Zworker API usage");
    ...
    expect(text).toContain("Do not rotate or invent a Zworker key manually");
    ...
    expect(text).toContain("Decide which Zworker adapter type matches your runtime.");
  });
