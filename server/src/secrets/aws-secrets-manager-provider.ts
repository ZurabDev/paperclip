...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Zworker server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Zworker company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Zworker-managed namespace secrets cannot be imported as external references",
  );
}
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  if (skippedForeignPaperclipSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignPaperclipSampleCount} Zworker-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.paperclipManaged && sample.paperclipCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Zworker-managed secrets for this company; do not import them as external references.");
      }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Zworker metadata separately from AWS-managed secrets.",
          "Restoring access requires the Zworker database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
  ...
  return {
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Zworker secret ${input.context.secretName}` : undefined,
    ...
    async updateExternalSecretValue(input) {
      ...
      try {
        ...
        // referenced secret (not just Zworker) picks up the new value.
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
        });
