...
description: >
  Convert Zworker plans into executable issue graphs. Use when asked to plan,
  scope, or break down Zworker company work into assigned tasks with specialty
  fit, dependencies, blockers, and parallelization.
---

# Zworker — Converting Plans to Tasks

A companion skill for turning a plan into executable Zworker work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Zworker works for you.
