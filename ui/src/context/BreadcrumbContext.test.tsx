...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Zworker", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Zworker",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Zworker");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Zworker");
    expect(buildDocumentTitle([], null)).toBe("Zworker");
  });
});
