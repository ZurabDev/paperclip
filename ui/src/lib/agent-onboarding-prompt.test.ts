...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Zworker API key");
    ...
    expect(prompt).toContain("Zworker maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Zworker host");
    expect(prompt).toContain("Hermes-originated Zworker API calls");
    expect(prompt).toContain("Do not rotate or invent a Zworker key manually");
