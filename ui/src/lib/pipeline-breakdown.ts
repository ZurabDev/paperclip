...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Zworker will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Zworker handles this" mechanics bullets, composed from the
