...
function makeBuiltInAgentState(overrides: Partial<BuiltInAgentState> = {}): BuiltInAgentState {
  return {
    definition: {
      ...
      defaultInstructions: "You are Zworker's built-in Briefs agent.",
...
describe("Agents", () => {
  ...
  it("gives mobile agent names the full row width after the leading status indicator", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({
        id: "agent-mobile",
        name: "Zworker Engineer With A Much Longer Display Name",
    ...
    const row = findAgentRow(container, "Zworker Engineer With A Much Longer Display Name");
    ...
    const name = row?.querySelector("span[title='Zworker Engineer With A Much Longer Display Name']");
  ...
  it("uses configured names for local-driver environments", async () => {
    ...
    expect(container.textContent).toContain("Zworker host");
  });
  ...
  it("shows and filters built-in agents when the experimental flag is enabled", async () => {
    ...
    expect(container.textContent).toContain("Briefs Agent");
    expect(container.textContent).not.toContain("Regular Agent");
    expect(container.querySelector('[title="Ships with Zworker"]')).toBeNull();
