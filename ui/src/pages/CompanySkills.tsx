...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Zworker", managedLabel: "Zworker managed" };
    default:
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      ...
      tooltip: "Zworker could not validate this skill as Agent Skills markdown. Install at your own risk.",
...
function buildDiscoveryCards(
  ...
  for (const entry of catalog) {
    ...
    cards.push({
      ...
      name: entry.name,
      slug: entry.slug,
      author: entry.packageName ?? "Zworker",
...
export function CompanySkills() {
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: `${skill.name} is now editable in the Zworker workspace.`,
      });
    },
    onError: (error) => {
  ...
  const studioDescription = studioForkFromId
    ...
    : "Create an editable company skill in the Zworker workspace.";
  return (
    <>
      {policyDenial.denial ? (
